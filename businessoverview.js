class BusinessOverview {
    ...
    bindEvents() {
        ...
        // EOD Export PDF
        const exportPdfBtn = document.getElementById('export-eod-pdf-btn');
        if (exportPdfBtn) {
            exportPdfBtn.addEventListener('click', () => this.exportEODReportToPdf());
        }
    ...
    async viewHistoricalSales() {
        ...
        try {
            ...
            if (eodReports && eodReports.length > 0) {
                ...
                // Store for PDF export filename
                this._lastEODBranchName = branchName;
                this._lastEODDate = date;
                const exportPdfBtn = document.getElementById('export-eod-pdf-btn');
                if (exportPdfBtn) {
                ...
                this._lastEODBranchName = null;
                this._lastEODDate = null;
                const exportPdfBtn = document.getElementById('export-eod-pdf-btn');
                if (exportPdfBtn) exportPdfBtn.style.display = 'none';
    ...
    /**
     * Export the currently displayed EOD report to PDF (Business Overview Historical Sales).
     * Uses jsPDF directly from the visible #historical-results DOM to avoid blank PDFs from html2canvas.
     */
    exportEODReportToPdf() {
        const container = document.getElementById('historical-results');
        if (!container || !container.querySelector('.eod-report-content')) {
            alert('No report to export. Please select a branch and date, then click "View Sales Data" first.');
            return;
        }
        ...
        const branchName = (this._lastEODBranchName || 'Branch').replace(/[^\w\s-]/g, '').replace(/\s+/g, '-') || 'Branch';
        const dateStr = (this._lastEODDate || '').replace(/\//g, '-') || new Date().toISOString().split('T')[0];
        const filename = `EOD-Report-${branchName}-${dateStr}.pdf`;
        ...
        try {
            ...
            // Strip emoji and other non-ASCII so Helvetica renders clean text (no garbled symbols)
            function stripForPdf(str) {
                if (typeof str !== 'string') return '';
                return str.replace(/[^\x20-\x7E\n\r\t]/g, '').replace(/\s+/g, ' ').trim();
            }

            function ensureSpace(linesNeeded = 1) {
                const estimatedHeight = linesNeeded * 6;
            ...
            function line(text, size = 11, bold = false) {
                ...
                doc.text(stripForPdf(String(text)), leftX, y);
                y += size >= 14 ? 8 : 6;
            }
            ...
            function keyValue(label, value) {
                ...
                doc.setFontSize(11);
                doc.text(stripForPdf(String(label)), leftX, y);
                doc.setFont('helvetica', 'normal');
                doc.text(stripForPdf(String(value)), valueX, y);
                y += 6;
            }

            // Header from DOM or fallback
            const headerEl = container.querySelector('.eod-report-header h4');
            const dateEl = container.querySelector('.eod-report-header .report-date');
            line('LIGI Reporting - EOD Report', 16, true);
            line(headerEl ? headerEl.textContent.trim() : `Branch: ${this._lastEODBranchName || 'Branch'}`, 11);
            line(dateEl ? dateEl.textContent.trim() : `Date: ${dateStr}`, 11);
            y += 2;

            // Walk all sections and eod-items in the visible report
            container.querySelectorAll('.eod-section').forEach(section => {
                const h5 = section.querySelector('h5');
                if (h5) {
                    line(h5.textContent.trim(), 13, true);
                }
                section.querySelectorAll('.eod-item').forEach(item => {
                    const labelEl = item.querySelector('.eod-label');
                    const valueEl = item.querySelector('.eod-value');
                    const label = labelEl ? labelEl.textContent.trim() : '';
                    const value = valueEl ? valueEl.textContent.trim() : '';
                    if (label || value) keyValue(label, value);
                });
                y += 2;
            });

            // Notes if present
            const notesEl = container.querySelector('.eod-notes');
            if (notesEl && notesEl.textContent.trim()) {
                line('Notes', 13, true);
                const notesText = notesEl.textContent.trim().replace(/[^\x20-\x7E\n\r]/g, '');
                const split = doc.splitTextToSize(notesText, 180);
                ensureSpace(split.length + 1);
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(11);
            ...
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(9);
            doc.text(stripForPdf('Generated by LIGI Reporting on ' + new Date().toLocaleString('en-US')), leftX, y);

            doc.save(filename);
        } catch (err) {
            console.error('PDF export failed:', err);
            alert('Failed to generate PDF. Error: ' + (err && err.message ? err.message : String(err)));
        }
    }
