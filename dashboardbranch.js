  ...
  if (eodBtn && eodModal && closeEodModal) {
      ...
      if (eodExportPdfBtn) {
        eodExportPdfBtn.addEventListener('click', function exportEODReportFromBranchModal() {
          ...
          if (!jsPDFCtor) {
            alert('PDF export library is not available. Please refresh and try again.');
            return;
          }
          ...
          try {
            ...
            function ensureSpace(linesNeeded) {
              var estimatedHeight = linesNeeded * 6;
            ...
            writeLine('LIGI Reporting - End of Day Report', 16, true);
            writeLine('Branch: ' + ((document.getElementById('eod-branch-date') ? document.getElementById('eod-branch-date').textContent : branchName) || 'Branch'), 11, false);
            writeLine('Date: ' + new Date(dateStr).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }), 11, false);
            y += 2;

            writeLine('Sales by Department', 13, true);
            if (departmentRows.length === 0) {
              writeLine('No department sales entered.', 11, false);
            } else {
              document.querySelectorAll('#eod-department-fields > div').forEach(function(div) {
                var label = div.querySelector('label');
                var input = div.querySelector('.eod-dept-sales');
                var deptName = label ? label.textContent.replace(/\s*Sales\s*\(\$\)\s*$/, '').trim() : 'Department';
                var amount = input ? (parseFloat(input.value) || 0) : 0;
                writeKeyValue(deptName + ':', '$' + amount.toLocaleString('en-US', { minimumFractionDigits: 2 }));
              });
            }
            writeKeyValue('Total Sales:', '$' + totalSales.toLocaleString('en-US', { minimumFractionDigits: 2 }));
            y += 2;

            writeLine('Financial Summary', 13, true);
            writeKeyValue('Total Sales:', '$' + totalSales.toLocaleString('en-US', { minimumFractionDigits: 2 }));
            writeKeyValue('Total Profit:', profitLossStr);
            writeKeyValue('Total Expenses:', '$' + expenses.toLocaleString('en-US', { minimumFractionDigits: 2 }));
            writeKeyValue('Net Revenue:', netRevStr);
            y += 2;

            writeLine('Payment Methods', 13, true);
            writeKeyValue('Cash on Hand:', '$' + cash.toLocaleString('en-US', { minimumFractionDigits: 2 }));
            writeKeyValue('Discrepancies:', '$' + discrepancies.toLocaleString('en-US', { minimumFractionDigits: 2 }));
            y += 2;

            writeLine('Financial Analysis', 13, true);
            writeKeyValue('Profit Margin:', profitMargin);
            writeKeyValue('Expense Ratio:', expenseRatio);

            if (notes && notes.trim()) {
              y += 2;
              writeLine('Notes', 13, true);
              var splitNotes = doc.splitTextToSize(notes.trim(), 180);
              ensureSpace(splitNotes.length + 1);
              doc.setFont('helvetica', 'normal');
              doc.setFontSize(11);
            ...
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(9);
            doc.text('Generated by LIGI Reporting on ' + new Date().toLocaleString('en-US'), leftX, y);
